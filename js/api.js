...
/* 톰캣 서버 주소 - 예전엔 "http://localhost:8080"으로 고정해 뒀었는데, 그러면 이 화면을
   내 컴퓨터가 아닌 다른 주소(예: 발표 때 강사님이 강사님 컴에서 내 IP로 접속하는 경우)로
   열면 API 호출은 계속 그 컴퓨터 자신의 localhost:8080을 찾아서 다 실패했다. 지금 이 페이지를
   불러온 주소(window.location.origin)를 그대로 쓰면 localhost로 열든 사설 IP로 열든
   항상 "그 페이지를 받아온 서버"로 API를 보내게 되어 따로 안 고쳐도 된다. */
